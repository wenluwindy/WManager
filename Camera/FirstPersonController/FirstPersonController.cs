...
namespace WManager
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private PhysicsMaterial woodPhysicMaterial;
        ...
        private PhysicsMaterial metalAndGlassPhysicMaterial;
        ...
        private PhysicsMaterial grassPhysicMaterial;
        ...
        private PhysicsMaterial dirtAndGravelPhysicMaterial;
        ...
        private PhysicsMaterial rockAndConcretePhysicMaterial;
        ...
        private PhysicsMaterial mudPhysicMaterial;
        ...
        private void Start()
        {
            ...
            zeroFrictionMaterial = new PhysicsMaterial("Zero_Friction")
            {
                dynamicFriction = 0f,
                staticFriction = 0f,
            ...
            highFrictionMaterial = new PhysicsMaterial("High_Friction")
            {
                dynamicFriction = 1f,
                staticFriction = 1f,
        ...
        private void FixedUpdate()
        {
            if (enableStamina)
            {
                isSprinting = Input.GetKey(sprintKey) && !isCrouching && staminaInternal > 0f
                    && (Mathf.Abs(mRigidBody.linearVelocity.x) > .01f || Mathf.Abs(mRigidBody.linearVelocity.z) > .01f);
                if (isSprinting) staminaInternal -= staminaDepletionSpeed * 2f * Time.deltaTime;
                else if ((!Input.GetKey(sprintKey) || Mathf.Abs(mRigidBody.linearVelocity.x) < .01f
                    || Mathf.Abs(mRigidBody.linearVelocity.z) < .01f || isCrouching) && staminaInternal < staminaLevel)
            ...
            if (maxSlopeAngle > 0f)
            {
                ...
                else
                {
                    ...
                    moveDirection = (transform.forward * input.y * speed + transform.right * input.x * walkSpeedInternal) * (mRigidBody.linearVelocity.y > .01f ? SlopeCheck() : .8f);
                }
            }
            ...
            yVelocity = mRigidBody.linearVelocity.y;
            if (IsGrounded && isJumping && jumpPowerInternal > 0f && !didJump)
            {
                if (maxSlopeAngle > 0f)
                {
                    if (isTouchingFlat || isTouchingWalkable)
                    {
                        didJump = true;
                        isJumping = false;
                        yVelocity += mRigidBody.linearVelocity.y < .01f ? jumpPowerInternal : jumpPowerInternal / 3;
            ...
            if (enablePlayerMovement) mRigidBody.linearVelocity = moveDirection + (Vector3.up * yVelocity);
            else mRigidBody.linearVelocity = Vector3.zero;
            ...
            if (fovKickAmount > 0f)
            {
                ...
                {
                    if (Mathf.Abs(mRigidBody.linearVelocity.x) > .5f || Mathf.Abs(mRigidBody.linearVelocity.z) > .5f)
            ...
            if (enableHeadbob)
            {
                head.localPosition = mRigidBody.linearVelocity.magnitude > .1f
