    ...
    internal class AssetBundleBuildTab
    {
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            m_ForceRebuild = new ToggleData(
                false,
                "清除文件夹",
                "如果选择在那里复制构建，将清除构建目录以及StreamingAssets/AssetBundles里的所有内容。",
                m_UserData.m_OnToggles);
        ...
        internal void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory))
            {
                ...
                var newPath = EditorGUILayout.TextField("输出路径", m_UserData.m_OutputPath);
        ...
        private void ExecuteBuild()
        {
            if (AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)
            {
                ...
                {
                    Debug.LogError("AssetBundle Build: 没有有效的生成输出路径。");
                    return;
                }

                if (m_ForceRebuild.state)
                {
                    string message = "是否要删除目录中的所有文件 " + m_UserData.m_OutputPath;
                    if (m_CopyToStreaming.state)
                        message += " 和 " + m_streamingPath;
                    message += "?";
                    if (EditorUtility.DisplayDialog("文件删除确认", message, "是", "否"))
                    {
                        try
                        {
